...
public class Main {
	...
	private static void execucaoMediaMmre() 
	{
		...
		double mMmre; //média dos valores de mmre encontrados em cada árvore-resultado da GP 
		...
		for (int i = 1; i < 13; i++)
		{
			...
			try 
			{
				...
				for (int j = 1; j <= numeroResultados; j++)
				{
					...
					mMmre += proximaGeracao.getIndividuos().get(0).getAptidao();
				...
				out.println("");
				out.println("\n\nMédia dos valores de mmre: " + (mMmre/numeroResultados));
				out.flush();
				out.close();
			}
			catch (Exception E)
	...
	/**
	 * Execução tradicional da programação genética, com a escolha de um dataset (chamada de método para preenchimento), chamada dos métodos de GP e a exibição da árvore de melhor aptidão da última geração e seus valores de MMRE e PRED.
	 */
